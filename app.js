/* ═══════════════════════════════════════════════════════════
   ELAYON · PRESENÇA V3.2
   Áudio bruto → métricas locais → Núcleo CRS → IA calibrada
═══════════════════════════════════════════════════════════ */
...
/* ── Estado global ── */
var userId = null, accessToken = null, tokens = 0, MAX_TOKENS = 50;
var tipoAnalise = 'unica', calibRef = null;
var micRec = null, micStream = null, micChunks = [], micGravando = false;
var micTimerInt = null, micSegundos = 0;
var MAX_SESSOES = 3, sessoesFifo = [];
var tokensConsumidos = 0, PERIODI_CADA = 4, calibPeriodRod = false;

/* ── Config IA ── */
var iaHabilitada = false, apiEndpointIA = '', iaProvedor = 'gemini', iaChave = '';
var analisesSalvas = [], MAX_ANALISES = 3;
...
var IA_PROVEDORES = {
  gemini: {
    ...
    body: function(sys, hist) {
      ...
      return JSON.stringify({ system_instruction: { parts: [{ text: sys }] }, contents: contents, generationConfig: { maxOutputTokens: 800 } });
    },
    extrair: function(d) { return d.candidates && d.candidates[0] && d.candidates[0].content ? d.candidates[0].content.parts[0].text : '—'; }
  },
  groq: {
  ...
  mistral: {
    ...
    headers: function(k) { return { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + k }; },
    body: function(sys, hist) { return JSON.stringify({ model: 'mistral-small-latest', max_tokens: 800, messages: [{ role: 'system', content: sys }].concat(hist) }); },
    extrair: function(d) { return d.choices && d.choices[0] ? d.choices[0].message.content : '—'; }
  },
  claude: {
    ...
    body: function(sys, hist) { return JSON.stringify({ model: 'claude-haiku-4-5-20251001', max_tokens: 800, system: sys, messages: hist }); },
    extrair: function(d) { return d.content && d.content[0] ? d.content[0].text : '—'; }
  },
  openai: {
    urlFn: function() { return 'https://api.openai.com/v1/chat/completions'; },
    headers: function(k) { return { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + k }; },
    body: function(sys, hist) { return JSON.stringify({ model: 'gpt-4o-mini', max_tokens: 800, messages: [{ role: 'system', content: sys }].concat(hist) }); },
    extrair: function(d) { return d.choices && d.choices[0] ? d.choices[0].message.content : '—'; }
  }
};
...
function carregarConfigIA() {
  ...
  if (cfg) {
    try {
      var p = JSON.parse(cfg);
      iaHabilitada = p.enabled || false;
      apiEndpointIA = p.endpoint || '';
      iaProvedor = p.provedor || 'gemini';
      iaChave = p.chave || '';
    } catch(e) {}
  }
  ...
  renderizarNotaProvedor();
}

function renderizarNotaProvedor() {
  var provEl = document.getElementById('iaProvedor');
  var notaEl = document.getElementById('iaProvedorNota');
  if (!provEl || !notaEl) return;
  var notas = {
    gemini:  'Gratuito · aistudio.google.com/apikey',
    groq:    'Gratuito · console.groq.com',
    mistral: 'Gratuito · console.mistral.ai',
    claude:  'Pago · console.anthropic.com',
    openai:  'Pago · platform.openai.com/api-keys'
  };
  notaEl.textContent = notas[provEl.value] || '';
}
...
function salvarConfiguracao() {
  ...
  localStorage.setItem('elayon_ia_config', JSON.stringify({ enabled: iaHabilitada, endpoint: apiEndpointIA, provedor: iaProvedor, chave: iaChave }));
...
function adicionarAnalise(dados, nome) {
  var id = 'a_' + Date.now();
  analisesSalvas.unshift({ id: id, timestamp: new Date().toLocaleString('pt-BR'), nome: nome || 'Análise ' + new Date().toLocaleTimeString('pt-BR'), dados: dados });
...
function atualizarUIAnalises() {
  var cont = document.getElementById('analisesList'); if (!cont) return;
  if (!analisesSalvas.length) { cont.innerHTML = '<p style="font-size:12px;color:var(--muted);text-align:center;padding:20px 0;">Nenhuma análise salva ainda</p>'; return; }
  cont.innerHTML = analisesSalvas.map(function(a) {
    return '<div class="analise-item"><span class="analise-nome">' + escHtml(a.nome) + '</span><span class="analise-data">' + a.timestamp + '</span><div class="analise-acoes"><button class="analise-btn" onclick="renomearAnalise(\'' + a.id + '\')">✎</button><button class="analise-btn" onclick="baixarAnalise(\'' + a.id + '\')">⬇</button><button class="analise-btn" onclick="removerAnalise(\'' + a.id + '\')">✕</button></div></div>';
  }).join('');
}

function renomearAnalise(id) {
  var a = analisesSalvas.find(function(x) { return x.id === id; }); if (!a) return;
...
function baixarAnalise(id) {
  ...
  link.href = url; link.download = a.nome.replace(/\s+/g,'_') + '.json'; link.click(); URL.revokeObjectURL(url);
}

function removerAnalise(id) {
  if (!confirm('Remover?')) return;
  analisesSalvas = analisesSalvas.filter(function(x) { return x.id !== id; });
  localStorage.setItem('elayon_analises', JSON.stringify(analisesSalvas)); atualizarUIAnalises();
}

async function enviarParaIA(payload) {
  if (!iaHabilitada || !apiEndpointIA) return;
  try {
    var res = await fetch(apiEndpointIA, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + accessToken }, body: JSON.stringify(payload) });
    if (res.ok) { adicionarAnalise(payload, 'Análise IA - ' + new Date().toLocaleTimeString('pt-BR')); return await res.json(); }
...
(async function boot() {
  ...
  if (f1 && f2 && f3 && !calibNestaSessao) {
    ...
    localStorage.removeItem('audio_fase_1'); localStorage.removeItem('audio_fase_2'); localStorage.removeItem('audio_fase_3');
  } else if (calibRef && calibNestaSessao) {
    abrirMic(); atualizarHudCalib();
  } else { mostrarCalibracao(); }
  carregarConfigIA();
  atualizarUI_tokens();
  chatBoot();
})();
...
async function carregarTokens() {
  ...
  tokens = r.data ? r.data.tokens : 0; atualizarUI_tokens();
}
...
function atualizarUI_tokens() {
  ...
  document.getElementById('tokensBar').style.width = Math.min(100, (tokens / MAX_TOKENS) * 100) + '%';
  var custo = tipoAnalise === 'unica' ? 1 : 2;
  var btn = document.getElementById('btnAnalisar');
  if (btn) btn.disabled = !(sessoesFifo.length > 0 && tokens >= custo);
  var aviso = document.getElementById('tokenSemAviso');
  if (aviso) aviso.style.display = (sessoesFifo.length > 0 && tokens < custo) ? 'block' : 'none';
}

async function creditarTokens(qtd) {
  var r = await supa.rpc('adicionar_tokens', { p_user_id: userId, p_quantidade: qtd });
  if (!r.error) { tokens += qtd; atualizarUI_tokens(); } return !r.error;
}

async function debitarTokens(qtd) {
  var r = await supa.rpc('adicionar_tokens', { p_user_id: userId, p_quantidade: -qtd });
  if (!r.error) { tokens -= qtd; tokensConsumidos += qtd; localStorage.setItem('elayon_tokens_consumidos', tokensConsumidos); atualizarUI_tokens(); verificarCalibracaoPeriodica(); }
  return !r.error;
}
...
async function verificarCalibracaoPeriodica() {
  if (calibPeriodRod || tokensConsumidos < PERIODI_CADA || !sessoesFifo.length) return;
  tokensConsumidos = 0; localStorage.setItem('elayon_tokens_consumidos', 0); calibPeriodRod = true;
  ...
  try {
    ...
    if (data.ok) { calibRef = { timestamp: new Date().toISOString(), tipo: 'periodica', fases: calibRef ? calibRef.fases : null, ref: novoRef, crs: data }; localStorage.setItem('elayon_calib_ref', JSON.stringify(calibRef)); atualizarHudCalib(); }
  } catch(e) { await creditarTokens(2); calibPeriodRod = false; return; }
...
/* ══════════════════════════════════════════════════════════
   CALIBRAÇÃO INICIAL
══════════════════════════════════════════════════════════ */
function mostrarCalibracao() { document.getElementById('calibAba').classList.remove('fechada'); document.getElementById('micCard').classList.add('hidden'); }
function abrirMic() { document.getElementById('calibAba').classList.add('fechada'); document.getElementById('micCard').classList.remove('hidden'); renderizarSessoes(); }

async function processarCalibracaoInicial() {
  setStatus('Processando calibração — aguarde...');
  var f1 = localStorage.getItem('audio_fase_1'), f2 = localStorage.getItem('audio_fase_2'), f3 = localStorage.getItem('audio_fase_3');
  if (!f1 || !f2 || !f3) return;
  var v1 = await extrairMetricasDeAudio(f1, 'Fase 01'); v1.context = 'calibracao_fase_01_estado_emissor';
  var v2 = await extrairMetricasDeAudio(f2, 'Fase 02'); v2.context = 'calibracao_fase_02_intencao';
  var v3 = await extrairMetricasDeAudio(f3, 'Fase 03'); v3.context = 'calibracao_fase_03_comprometimento';
  var r1 = await enviarAoRender(v1), r2 = await enviarAoRender(v2), r3 = await enviarAoRender(v3);
  ...
  calibRef = { timestamp: new Date().toISOString(), tipo: 'inicial', fases: { f1: { vetor: v1, crs: r1 }, f2: { vetor: v2, crs: r2 }, f3: { vetor: v3, crs: r3 } }, ref: refVetor, refCrs: refCrs };
  localStorage.setItem('elayon_calib_ref', JSON.stringify(calibRef));
  sessionStorage.setItem('elayon_calibrado', '1');
  await creditarTokens(2); setStatus(''); atualizarHudCalib(); abrirMic();
...
function atualizarHudCalib() {
  var el = document.getElementById('hudCalibTs'); if (!calibRef || !el) return;
  var d = new Date(calibRef.timestamp);
  el.textContent = 'CAL ' + (calibRef.tipo === 'periodica' ? '↺' : '✓') + ' ' + d.toLocaleTimeString('pt-BR', { hour: '2-digit', minute: '2-digit' });
}

function irCalibrar() {
  ['audio_fase_1','audio_fase_2','audio_fase_3'].forEach(function(k) { localStorage.removeItem(k); });
  sessionStorage.removeItem('elayon_calibrado'); window.location.href = CALIB_URL;
}
...
function micIniciar() {
  ...
  navigator.mediaDevices.getUserMedia({ audio: true }).then(function(s) {
    ...
    micRec.start(100); micGravando = true; micSegundos = 0;
...
function micParar() {
  ...
  clearInterval(micTimerInt); micGravando = false;
  micRec.onstop = async function() {
    ...
    renderizarSessoes(); atualizarUI_tokens();
  ...
  micRec.stop(); micStream.getTracks().forEach(function(t) { t.stop(); }); setWave(false);
...
function renderizarSessoes() {
  var cont = document.getElementById('sessoesFifo'); if (!cont) return;
  if (!sessoesFifo.length) { cont.innerHTML = ''; return; }
  ...
  sessoesFifo.forEach(function(s, i) {
    ...
    html += '<div class="sessao-item' + (i === 0 ? ' sessao-atual' : '') + '"><span class="sessao-idx">S' + (i+1) + (i === 0 ? ' ←' : '') + '</span><span class="sessao-info">' + hr + ' · ' + (s.duration_sec || 0) + 's</span></div>';
  });
  cont.innerHTML = html;
}
...
async function iniciarAnalise() {
  var custo = tipoAnalise === 'unica' ? 1 : 2;
  if (tokens < custo || !sessoesFifo.length) return;
  ...
  try {
    var sessaoAtual;
    if (tipoAnalise === 'unica') { sessaoAtual = sessoesFifo[0]; }
    else { var n = sessoesFifo.length; sessaoAtual = consolidarVetores(sessoesFifo, sessoesFifo.map(function() { return 1/n; })); sessaoAtual.context = 'leitura_memoria_' + n + '_sessoes'; }
    ...
    document.getElementById('cardResultado').innerHTML = '<span class="card-label">Erro de comunicação</span><p class="resultado-texto">Não foi possível conectar ao núcleo.</p><button class="btn btn-ghost" style="margin-top:16px;" onclick="document.getElementById(\'cardResultado\').classList.add(\'hidden\')">Fechar</button>';
...
function montarPayload(sessao, delta) {
  return {
    ...
    calibracao_ref: calibRef ? { timestamp: calibRef.timestamp, tipo: calibRef.tipo, ref: calibRef.ref,
...
async function enviarAoRender(payload) {
  var res = await fetch(CRS_ENDPOINT, { method: 'POST', headers: { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + accessToken }, body: JSON.stringify(payload) });
  return res.json();
}
...
function chatBoot() {
  ...
  input.addEventListener('keydown', function(e) { if (e.key === 'Enter' && !e.shiftKey) { e.preventDefault(); chatEnviar(); } });
...
async function chatEnviar() {
  ...
  try {
    ...
    var resposta = await chatChamarIA(system, CHAT_HISTORICO);
    ...
    chatAdicionarMensagem('ai', e.message || 'Não foi possível processar. Verifique a chave e o provedor.');
  }
...
function chatMontarVetorAtual() {
  ...
  return { duration_sec:30, silence_pct:20, pause_count:3, mean_pause_ms:300, oscillation_pct:25, continuity_pct:75, energy_pct:60, stability_pct:70, spectrum_snapshot:{ graves:50, medios:60, agudos:40, ruido:20, estabilidade:75 } };
}
...
function chatMontarSystem(rel) {
  ...
  var s = 'Você é uma presença acolhedora do sistema Elayon. Responda em português brasileiro. '
    + 'Seja direto, caloroso e humano. Seu único objetivo é ajudar o emissor a se ver com amor — '
    + 'a si mesmo e ao próximo. Conduza a conversa com suavidade e intenção. '
  ...
  if (estado && instrucao) s += '\n\n[LEITURA INTERNA — NÃO REVELAR]: Estado: ' + estado + '. Instrução: ' + instrucao;
  return s;
}

async function chatChamarIA(system, historico) {
  if (!iaChave) iaChave = localStorage.getItem('elayon_claude_key') || '';
  if (!iaChave) throw new Error('Chave da IA não configurada. Vá em Configuração de Fluxo, insira a chave e salve.');
  var prov = IA_PROVEDORES[iaProvedor];
  if (!prov) throw new Error('Provedor desconhecido: ' + iaProvedor);
  var url = prov.urlFn(iaChave);
  var res = await fetch(url, { method: 'POST', headers: prov.headers(iaChave), body: prov.body(system, historico) });
  if (!res.ok) { var err = await res.text(); throw new Error('Erro ' + res.status + ': ' + err.slice(0, 120)); }
  return prov.extrair(await res.json());
}

function chatAdicionarMensagem(role, texto) {
  var wrap = document.getElementById('chatMensagens'); if (!wrap) return;
  var div = document.createElement('div'); div.className = 'chat-msg chat-' + role;
  var bubble = document.createElement('div'); bubble.className = 'chat-bubble'; bubble.textContent = texto;
  ...
  div.appendChild(bubble); div.appendChild(meta); wrap.appendChild(div);
...
function chatMostrarTyping(mostrar) {
  ...
  if (!mostrar) return;
  var div = document.createElement('div'); div.id = 'chatTyping'; div.className = 'chat-msg chat-ai';
  div.innerHTML = '<div class="chat-bubble chat-typing"><span class="typing-dot"></span><span class="typing-dot"></span><span class="typing-dot"></span></div>';
  wrap.appendChild(div); wrap.scrollTop = wrap.scrollHeight;
}

function chatSetCrsStatus(estado) {
  var dot = document.getElementById('chatCrsDot'), label = document.getElementById('chatCrsLabel');
  if (!dot || !label) return;
...
function exibirRelatorioVisual(opts) {
  var card = document.getElementById('cardResultado'); card.classList.remove('hidden');
  var s = opts.sessao || {}, rel = (opts.crs && opts.crs.relatorio) ? opts.crs.relatorio : {}, snap = rel.snapshot_sonoro || s.spectrum_snapshot || {};
  var silP = rel.porcentagem_silencio || s.silence_pct || 0, engP = rel.energia_pct || s.energy_pct || 0;
  var oscP = rel.oscilacao_pct || s.oscillation_pct || 0, contP = rel.continuidade_pct || s.continuity_pct || 0;
  var dur = rel.tempo_total || s.duration_sec || 0, pausas = rel.total_pausas || s.pause_count || 0;
  var html = '<div class="estado-tag">' + escHtml(opts.titulo || '—') + '</div>';
  html += '<div class="resultado-bloco"><h4>' + (opts.ehCalib ? 'Referência registrada' : 'Instrução à IA receptora') + '</h4><p class="resultado-texto">' + escHtml(opts.subtitulo || '—') + '</p></div><div class="sep"></div>';
  ...
  [{ l:'Silêncio', v:silP, max:100, u:'%', c:'#3ecfcf' }, { l:'Energia', v:engP, max:100, u:'%', c:'#2a9d8f' },
   ...
   { l:'Pausas', v:pausas, max:20, u:'', c:'#3ecfcf' }, { l:'Duração', v:dur, max:60, u:'s', c:'#4a7a80' }
    ...
    html += '<div class="metrica-card"><div class="metrica-label">' + m.l + '</div><div class="metrica-val">' + m.v + '<span>' + m.u + '</span></div><div class="metrica-bar-wrap"><div class="metrica-bar" style="width:' + pct + '%;background:' + m.c + '"></div></div></div>';
  });
  ...
  [{ l:'Graves', v:snap.graves||0, c:'#2a9d8f' }, { l:'Médios', v:snap.medios||0, c:'#3ecfcf' },
   ...
   { l:'Estab.', v:snap.estabilidade||0, c:'#2a9d8f' }
    ...
    html += '<div class="espectro-col"><div class="espectro-bar-wrap"><div class="espectro-bar" style="height:' + b.v + '%;background:' + b.c + '"></div></div><div class="espectro-label">' + b.l + '</div><div class="espectro-val">' + b.v + '%</div></div>';
  });
  html += '</div></div><div class="sep"></div>';
  html += '<div class="resultado-bloco"><h4>Perfil rítmico — radar CRS</h4><canvas id="radarCanvas" width="260" height="260" style="display:block;margin:10px auto 0;"></canvas></div><div class="sep"></div>';
  html += '<div class="resultado-bloco"><h4>Mapa de presença</h4><canvas id="silCanvas" width="440" height="56" style="width:100%;border-radius:8px;margin-top:8px;"></canvas><div style="display:flex;justify-content:space-between;margin-top:6px;"><span style="font-family:var(--mono);font-size:8px;color:var(--teal)">■ Voz</span><span style="font-family:var(--mono);font-size:8px;color:var(--muted)">■ Silêncio · ' + silP + '%</span></div></div>';
  if (opts.delta) {
    html += '<div class="sep"></div><div class="resultado-bloco"><h4>Delta vs calibração</h4><div class="delta-grid">';
    [{ l:'Silêncio', v:opts.delta.silence_pct }, { l:'Energia', v:opts.delta.energy_pct }, { l:'Oscilação', v:opts.delta.oscillation_pct }, { l:'Continuidade', v:opts.delta.continuity_pct }].forEach(function(d) {
      var pos = d.v >= 0;
      html += '<div class="delta-item"><span class="delta-label">' + d.l + '</span><span class="delta-val ' + (pos?'pos':'neg') + '">' + (pos?'+':'') + d.v + '</span></div>';
    });
    html += '</div></div>';
  }
  if (opts.calibRef && opts.calibRef.fases) {
    ...
    ['f1','f2','f3'].forEach(function(fn) {
      ...
      html += '<div class="fase-card"><span class="fase-num">' + fn.toUpperCase() + '</span><span class="fase-nome">' + fnomes[fn] + '</span><span class="fase-estado">' + escHtml(est) + '</span></div>';
    });
    html += '</div></div>';
  }
...
function calcularDelta(sessao, ref) {
  if (!ref) return null;
  var delta = {};
  ['silence_pct','energy_pct','oscillation_pct','continuity_pct','pause_count','mean_pause_ms'].forEach(function(c) { delta[c] = parseFloat(((sessao[c]||0) - (ref[c]||0)).toFixed(2)); });
  return delta;
}

function setStatus(msg) { var el = document.getElementById('extracaoStatus'); if (el) el.textContent = msg; }

function blobToDataUrl(blob) { return new Promise(function(res) { var r = new FileReader(); r.onload = function(e) { res(e.target.result); }; r.readAsDataURL(blob); }); }
...
async function base64ToArrayBuffer(dataUrl) {
  ...
  for (var i = 0; i < bin.length; i++) view[i] = bin.charCodeAt(i); return buf;
}
...
async function extrairMetricasDeAudio(dataUrl, label) {
  ...
  for (var k=0; k<rms.length; k++) { if (rms[k]<THR) { silF++; inP ? pLen++ : (inP=true, pLen=1); } else { if(inP && pLen>=MF){ pC++; pLens.push(pLen*25); } inP=false; pLen=0; } }
  var silP = rms.length ? (silF/rms.length)*100 : 0, meanP = pLens.length ? pLens.reduce(function(a,b){return a+b;},0)/pLens.length : 0;
