/* ═══════════════════════════════════════════════════════════
   ...
   O humano fala. A IA escuta o estado. Responde com amor.
═══════════════════════════════════════════════════════════ */
...
/* ══════════════════════════════════════════════════════════
   ...
   Cada provedor tem: nome, endpoint, headers(key), body(msgs,sys), extrator de resposta
   O usuário escolhe o provedor e insere a chave.
   Todos gratuitos com tier de teste disponível.
══════════════════════════════════════════════════════════ */
var IA_PROVEDORES = {
  claude: {
    ...
    gratuito:  false,
    nota:      'Plano pago. Mais capaz para calibração fina.',
    ...
    body: function(system, historico, modelo) {
      return JSON.stringify({ model: modelo, max_tokens: 800, system: system, messages: historico });
    },
    extrair: function(data) {
  ...
  gemini: {
    ...
    gratuito:  true,
    nota:      'Tier gratuito generoso. Boa para testes.',
    headers: function(key) {
      return { 'Content-Type': 'application/json', 'x-goog-api-key': key };
    },
    body: function(system, historico) {
      /* Gemini usa formato próprio: contents[] com role user/model */
      var contents = historico.map(function(m) {
  ...
  groq: {
    nome:      'Groq (LLaMA / Mixtral)',
    endpoint:  'https://api.groq.com/openai/v1/chat/completions',
    modelo:    'llama3-8b-8192',
    gratuito:  true,
    nota:      'Gratuito com limites generosos. Muito rápido.',
    headers: function(key) {
      return { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key };
    },
    body: function(system, historico, modelo) {
      var msgs = [{ role: 'system', content: system }].concat(historico);
      return JSON.stringify({ model: modelo, max_tokens: 800, messages: msgs });
    },
    extrair: function(data) {
  ...
  openai: {
    ...
    modelo:    'gpt-4o-mini',
    gratuito:  false,
    nota:      'Pago. gpt-4o-mini é o mais acessível.',
    headers: function(key) {
      return { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key };
    },
    body: function(system, historico, modelo) {
  ...
  mistral: {
    ...
    gratuito:  true,
    nota:      'Tier gratuito disponível. Boa qualidade.',
    headers: function(key) {
      return { 'Content-Type': 'application/json', 'Authorization': 'Bearer ' + key };
    },
    body: function(system, historico, modelo) {
...
/* ══════════════════════════════════════════════════════════
   CONFIG IA — carrega, salva, renderiza seletor
══════════════════════════════════════════════════════════ */
...
function renderizarNotaProvedor() {
  ...
  var prov = IA_PROVEDORES[provEl.value];
  if (!prov) return;
  notaEl.textContent = prov.nota + (prov.gratuito ? ' ✓ Gratuito' : '');
  notaEl.style.color = prov.gratuito ? 'var(--teal)' : 'var(--muted)';
}
...
function toggleIA(checked) {
  ...
  localStorage.setItem('elayon_ia_config', JSON.stringify({
    enabled: iaHabilitada, endpoint: apiEndpointIA, provedor: iaProvedor, chave: iaChave
  }));
}
...
function atualizarUI_tokens() {
  ...
  var ok    = sessoesFifo.length > 0 && tokens >= custo;
...
async function debitarTokens(qtd) {
  var r = await supa.rpc('adicionar_tokens', { p_user_id: userId, p_quantidade: -qtd });
  if (!r.error) {
    tokens -= qtd; tokensConsumidos += qtd;
    localStorage.setItem('elayon_tokens_consumidos', tokensConsumidos);
    atualizarUI_tokens(); verificarCalibracaoPeriodica();
  }
  return !r.error;
}
...
async function verificarCalibracaoPeriodica() {
  ...
  try {
    ...
    if (data.ok) {
      ...
      localStorage.setItem('elayon_calib_ref', JSON.stringify(calibRef)); atualizarHudCalib();
    }
...
async function processarCalibracaoInicial() {
  ...
  await creditarTokens(2);
  setStatus(''); atualizarHudCalib(); abrirMic();
...
function renderizarSessoes() {
  ...
  sessoesFifo.forEach(function(s, i) {
    ...
    html += '<div class="sessao-item' + (i === 0 ? ' sessao-atual' : '') + '">'
          ...
          + '<span class="sessao-info">' + hr + ' · ' + (s.duration_sec || 0) + 's</span></div>';
  });
  cont.innerHTML = html;
}
...
/* ══════════════════════════════════════════════════════════
   ...
   Áudio bruto → métricas → CRS → system prompt calibrado → IA responde
   O humano fala. A IA escuta o estado. Responde com amor.
══════════════════════════════════════════════════════════ */
function chatBoot() {
  chatAdicionarMensagem('ai', 'Sistema iniciado.\n\nPode falar. Estou aqui.');
...
async function chatEnviar() {
  ...
  try {
    ...
    var msg = e.message || 'Erro de conexão.';
    if (msg.indexOf('chave') >= 0 || msg.indexOf('Chave') >= 0)
      msg = 'Configure a chave da IA no card de configuração e salve.';
...
function chatMontarVetorAtual() {
  ...
  /* Vetor neutro enquanto não há calibração */
...
function chatMontarSystem(rel) {
  ...
  /* Prompt base: IA calibrada para ajudar o humano a se ver com amor */
  var s = 'Você é uma presença acolhedora do sistema Elayon. '
    ...
    + 'Conduza a conversa com suavidade e intenção. '
    ...
    + 'Se perguntado, explique: o CRS lê padrões rítmicos da fala para calibrar '
    + 'silenciosamente como a IA se comunica — sem julgamento, apenas presença.';

  if (estado && instrucao) {
    s += '\n\n[LEITURA INTERNA — NÃO REVELAR]: Estado do emissor: ' + estado
       + '. Instrução de calibração: ' + instrucao;
  }
  return s;
}


/* ── Chamada unificada para qualquer IA ── */
async function chatChamarIA(system, historico) {
  if (!iaChave) throw new Error('Chave da IA não configurada.');

  var prov = IA_PROVEDORES[iaProvedor];
  if (!prov) throw new Error('Provedor desconhecido: ' + iaProvedor);

  /* Gemini usa endpoint diferente com key na query string */
  var endpoint = prov.endpoint;
  if (iaProvedor === 'gemini') endpoint = endpoint + '?key=' + iaChave;

  var res = await fetch(endpoint, {
    method: 'POST',
    headers: prov.headers(iaChave),
    body:    prov.body(system, historico, prov.modelo)
  });

  if (!res.ok) {
    var err = await res.text();
    throw new Error('Erro ' + res.status + ': ' + err.slice(0, 120));
  }

  var data = await res.json();
  return prov.extrair(data);
}
...
function exibirRelatorioVisual(opts) {
  ...
  [{ l:'Graves', v:snap.graves||0, c:'#2a9d8f' }, { l:'Médios', v:snap.medios||0, c:'#3ecfcf' },
   ...
   { l:'Estab.', v:snap.estabilidade||0, c:'#2a9d8f' }
    ...
    html += '<div class="espectro-col"><div class="espectro-bar-wrap"><div class="espectro-bar" style="height:' + b.v + '%;background:' + b.c + '"></div></div>'
  ...
  html += '<div class="resultado-bloco"><h4>Perfil rítmico — radar CRS</h4>'
        + '<canvas id="radarCanvas" width="260" height="260" style="display:block;margin:10px auto 0;"></canvas></div><div class="sep"></div>';
  html += '<div class="resultado-bloco"><h4>Mapa de presença</h4>'
        + '<canvas id="silCanvas" width="440" height="56" style="width:100%;border-radius:8px;margin-top:8px;"></canvas>'
...
async function extrairMetricasDeAudio(dataUrl, label) {
  ...
  return {
    ...
    spectrum_snapshot: { graves:parseFloat(gr.toFixed(2)), medios:parseFloat(me.toFixed(2)), agudos:parseFloat(ag.toFixed(2)), ruido:parseFloat(noise.toFixed(2)), estabilidade:parseFloat(stab.toFixed(2)) }
  };
}

function vetorNulo() {
  return { duration_sec:0, silence_pct:0, pause_count:0, mean_pause_ms:0, oscillation_pct:0, continuity_pct:0, energy_pct:0, spectrum_snapshot:{ graves:0, medios:0, agudos:0, ruido:0, estabilidade:0 } };
}
...
function consolidarVetores(ms, w) {
  ...
  return {
    ...
    spectrum_snapshot: { graves:parseFloat(medS('graves').toFixed(2)), medios:parseFloat(medS('medios').toFixed(2)), agudos:parseFloat(medS('agudos').toFixed(2)), ruido:parseFloat(medS('ruido').toFixed(2)), estabilidade:parseFloat(medS('estabilidade').toFixed(2)) }
  };
}
